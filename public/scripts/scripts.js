	...
	typeof window !== "undefined" ? window: this, function(window, noGlobal) {
		...
		*/
		jQuery.event = {
			...
			dispatch: function(nativeEvent) {
				...
				var i, j, ret, matched, handleObj, handlerQueue,
					...
					special = jQuery.event.special[event.typ] || {};
				...
				text: function(value) {
					return access(
						this, function(value) {
							...
							jQuery.text(this): this.empty().each(
								function() {
		...
		function getWidthOrHeight(elem, dimension, extra) {
			...
			if (
				(!support.boxSizingReliable() && isBorderBox || val === "auto" || !parseFloat(val) && jQuery.css(elem, "display", false, styles) === "inline") 
			...
			function(elem, computed) {
				if (computed) {
					return (
						...
						elem.getBoundingClientRect().left - swap(
				...
				var callbackName, overwritten, responseContainer,
					jsonProp = s.jsonp !== false && (
						rjsonp.test(s.url) ? "url":
							typeof s.data === "string" && (s.contentType || "").indexOf("application/x-www-form-urlencoded") === 0 &&
...
(
	function(window, factory) {
		...
		if (typeof define == 'function' && define.amd) {
			// AMD
			define(
				'unipointer/unipointer', ['ev-emitter/ev-emitter'], function(EvEmitter) {
